...
class CheckServcie(RequestSelf):
    def testapi(self):
        ...
        try:
            ...
            his.result = '' if not data else json.dumps(data)
            his.uuid = uuid
            self.create(his)
        except:
            #防止修改代码造成问题
    ...
    def runscr(self, project, uid,pname):
        scr = ScriptInfo.query.filter_by(isTiming=False, isDelete=False, projectId=project.id).all()
        if not scr:
            return False
...
class ProjectServer(RequestSelf):
    ...
    def addpu(self, user):
        ...
        if not puadmin and not flag:
            return 5
        if not puadmin.isAdmin and not flag:
            return 5
...
>错误原因:&#8194;{}    
>提交时间:&#8194;{}     
>检查平台:&#8194;[点击查看详情](http://10.225.136.151:20009/checks?id={})  
'''.format(color, info, project.name, res, author, errormsg, date, project.id)
...
>错误原因:&#8194;{} 
>提交时间:&#8194;{}  
>检查平台:   http://10.225.136.151:20009/checks?id={}  
        '''.format(color, info, '狼人对决', res, 'robin', errormsg, '2019-08-16 15:47:05 +0800', '13')
        ding = DingDingBot()
        ding.run(token=token, post_string=post_string)
        return
